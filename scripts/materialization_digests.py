#!/usr/bin/env python
"""Compute every package's materialization digests from its own committed lock.

    uv run python scripts/materialization_digests.py [--repo DIR] [--json]

Each package is materialized independently, from its own ``uv.lock``, for every
marker environment in ``ci/marker-environments.json``. There is no shared root
lock in this computation on purpose: the per-package lock is the identity source,
and the one-package-one-digest-change check below depends on that being true.
"""
...
from cruxible_provider_runtime.digests import materialization_digest  # noqa: E402
...
def compute(
    ...
    for package_dir in package_dirs(repo):
        ...
        digests[name] = {
            env.id: materialization_digest(resolve(lock, name, env)) for env in environments
        }
    return digests
