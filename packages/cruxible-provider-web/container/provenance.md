...
A per-engine extra changes what is installed, so it changes the materialization
the image corresponds to, so it changes which pin the accepted artifact must
carry (`<marker-environment>+<extra>`). Build one image per extras set the
deployment binds, and give each the materialization digest of *its* environment.
An image built without an engine and labelled with the engine environment's
digest would verify and then fail at the first run that needed the engine.
