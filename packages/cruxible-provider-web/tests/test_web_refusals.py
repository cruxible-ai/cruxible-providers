...
The generic ones are here because a plane package inherits the whole
conformance suite rather than trusting that the reference provider exercised
them; the extras-shaped ones are here because this is the first package whose
implementations bind to *different environments from one lock*, and that is
where a new class of mismatch becomes possible.
"""
...
def test_undeclared_interface_refuses(
    ...
    with pytest.raises(RefusalError) as exc:
        bind(
            registry,
            _request(manifest_path, lock_path, interface_id="web.crawl"),
