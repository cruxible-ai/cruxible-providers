"""Bind-time refusals, each one a rule the RP-0 contract states.

A plane package inherits the whole conformance suite rather than trusting that
the reference provider exercised these paths. The extras-shaped ones matter more
here than anywhere: both implementations in this package carry an engine, so
every bind is a bind of an engine environment and there is no base environment to
fall back to if a pin is missing.
"""
...
from .conftest import MARKDOWN_EXTRAS, MARKER_ENVIRONMENT, PROVIDER_ID


def _request(
    manifest_path: Path,
    lock_path: Path | None,
    interface_id: str = "doc.to_markdown",
...
def _reaccepted(
    ...
    """Re-transcribe an edited manifest, so a later check is the one under test."""
...
def test_manifest_divergence_refuses(
    ...
    def edit(document: dict[str, Any]) -> None:
        document["implementations"][0]["deterministic"] = True
...
def test_unknown_manifest_field_refuses(
    ...
    def edit(document: dict[str, Any]) -> None:
        document["implementations"][0]["engine"] = "playwright"
...
def test_undeclared_interface_refuses(
    ...
    with pytest.raises(RefusalError) as exc:
        bind(
            registry,
            _request(manifest_path, lock_path, interface_id="doc.to_pdf"),
...
def test_unsupported_backend_refuses(
    ...
    def edit(document: dict[str, Any]) -> None:
        for implementation in document["implementations"]:
            implementation["backends"] = ["local_env"]
...
def test_an_artifact_pinning_only_the_other_environment_refuses(
    accepted_artifact: ProviderArtifactPayload,
    manifest_path: Path,
    lock_path: Path,
    local_backend: LocalEnvBackend,
    ...
    engine_key = environment_pin_key(MARKER_ENVIRONMENT.id, MARKDOWN_EXTRAS)
    base_only = {
        key: digest
    ...
    assert exc.value.refusal.detail["extras"] == list(MARKDOWN_EXTRAS)
...
def test_an_extra_the_lock_does_not_declare_refuses(
    ...
    """A manifest may not invent an engine the distribution cannot install."""

    def edit(document: dict[str, Any]) -> None:
        document["implementations"][0]["requires_extras"] = ["telepathy"]
    ...
    assert exc.value.refusal.detail["extra"] == "telepathy"
